<?php
	...
	if (!empty($_SESSION['UserID']) && !empty($_POST))
	{
		...
		$TaskStr = 'UPDATE '.$DataBase['TablePrefix'].'Tasks SET UserID='.$_SESSION['UserID'].', ';
		...
		if (!empty($_POST['NotesI']))
		{
			$TaskStr .= "Notes='".mysql_real_escape_string($_POST['NotesI'])."', ";
		}
		if (!empty($Duration))
		{
			$TaskStr .= "Duration='".$Duration."', ";
		}
		if (!empty($DateDueEdit) && ($TimeStampDue = strtotime($DateDueEdit)) !== false)
		{
		...
		// 2Do:
		$TaskStr = substr(trim($TaskStr), 0, -1).' WHERE ID='.$TaskID.'';
		...
		if ($TaskRes)
		{
			echo "$('#Name,#Notes').val('');";
			echo "$('#Name').focus();";
			echo "$('#ResponseBox').html('Task updated.').fadeIn('slow').fadeTo(5000, 1).fadeOut('slow');";
			echo "$('#Menu > ul').tabs('url',1 , 'Task.php');";
	...
	{
		echo "$('#ResponseBox').html('ERROR: no session.').fadeIn('slow').fadeTo(5000, 1).fadeOut('99999')";
	}
?>